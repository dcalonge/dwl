...
struct Pertag {
	...
	int nmasters[sizeof(tags) + 1]; /* number of windows in master area */
	float mfacts[sizeof(tags) + 1]; /* mfacts per tag */
	unsigned int sellts[sizeof(tags) + 1]; /* selected layouts */
	const Layout *ltidxs[sizeof(tags) + 1][2]; /* matrix of tags and layouts indexes  */
};
